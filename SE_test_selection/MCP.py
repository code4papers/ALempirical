...
def select_my_optimize(model, selectsize, x_target, y_test):
    ...
    matrix_size = y_test.shape[-1]

    act_layers = model.predict(x_target)
    dicratio = [[] for i in range(matrix_size * matrix_size)]  # 只用90，闲置10个
    dicindex = [[] for i in range(matrix_size * matrix_size)]
    for i in range(len(act_layers)):
        act = act_layers[i]
        max_index, sec_index, ratio = find_second(act, matrix_size)  # max_index
        # 安装第一和第二大的标签来存储，比如第一是8，第二是4，那么就存在84里，比例和测试用例的序号
        dicratio[max_index * matrix_size + sec_index].append(ratio)
        dicindex[max_index * matrix_size + sec_index].append(i)

    selected_lst = select_from_firstsec_dic(selectsize, dicratio, dicindex, matrix_size * matrix_size)
    return selected_lst
    ...
    #
    # all_index = np.arange(len(act_layers))
    # remain_index = np.delete(all_index, selected_lst)
    #
    # x_remain = x_target[remain_index]
    # y_remain = y_test[remain_index]
    #
...
if __name__ == "__main__":
    ...
    acc_ls = []
    for i in range(5):
        model = load_model("models/ite-2.h5")
        data_path = "data/scale_combine.npy"
        acc = retrain_2(model, data_path, 1000)
        acc_ls.append(acc)
