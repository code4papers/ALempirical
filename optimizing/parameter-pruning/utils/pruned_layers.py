...
#
#         self.w = self.add_weight(shape=(n_neurons_in, self.n_neurons_out), initializer="glorot_normal", dtype=self.dtype,
#                                       name='weight', trainable=True)
#         self.b = self.add_weight(shape=(self.n_neurons_out,), initializer="zeros", name='bias', trainable=True)
#         self._trainable_weights = [self.w, self.b]
...
#         self.mask = self.add_weight(shape=(n_neurons_in, self.n_neurons_out), initializer="ones", dtype=self.dtype, name='mask',
#                                     trainable=False)
#         self._non_trainable_weights = [self.mask]
#
...
class pruned_Dense(Layer):
    ...
    def build(self, input_shape):
        ...
        # mask = np.ones((n_neurons_in, self.n_neurons_out))
        # self.mask = K.variable(mask)
        self.mask = self.add_weight(shape=(n_neurons_in, self.n_neurons_out), initializer="ones", dtype=self.dtype,
                                    name='mask',
                                    trainable=False)
        self._non_trainable_weights = [self.mask]
...
class pruned_Conv2D(Layer):
    ...
    def build(self, input_shape):
        ...
        self.mask = self.add_weight(shape=kernel_shape, initializer="ones", dtype=self.dtype, name='mask', trainable=False)
